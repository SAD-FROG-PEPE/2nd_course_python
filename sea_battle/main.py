...
menu_y = 50
ships = s_x // 2  # максимальное кол-во кораблей
...
canvas.create_rectangle(0, 0, size_canvas_x, size_canvas_y, fill="misty rose")
...
b0.place(x=size_canvas_x + menu_x // 2 - b0.winfo_reqwidth() // 2, y=20)
b1 = Button(tk, text="Показать корабли \n Игрока №2", command=button_show_enemy2)
b1.place(x=size_canvas_x + menu_x // 2 - b1.winfo_reqwidth() // 2, y=70)
b2 = Button(tk, text="Начать заново!", command=button_begin_again)
b2.place(x=size_canvas_x + menu_x // 2 - b2.winfo_reqwidth() // 2, y=200)
...
def hod_computer():
    ...
    if check_winner1():
        ...
        id3 = canvas.create_text(size_canvas_x + menu_x // 2, 300, text=winner, font=("Times", 10), justify=CENTER)
        list_ids.append(id3)
    ...
    if ip_x < s_x and ip_y < s_y and hod_igrovomu_polu_1:
        if points1[ip_y][ip_x] == -1:
            ...
            if check_winner1():
                ...
                points1 = [[10 for i in range(s_x)] for i in range(s_y)]
                points2 = [[10 for i in range(s_x)] for i in range(s_y)]
                id3 = canvas.create_text(size_canvas_x + menu_x // 2, 300,
                                         text=winner, font=("Times", 10), justify=CENTER)
                list_ids.append(id3)

    # второе игровое поле
    if s_x + delta_menu_x <= ip_x <= s_x + s_x + delta_menu_x and ip_y < s_y and not hod_igrovomu_polu_1:
